program dg
  ...
  ! Main variables
  !real(kind=8),dimension(1:nvar,1:n,1:nx)::u,dudt,w1,w2,w3,w4,w5,w6, ureal
  !real(kind=8),dimension(1:nvar,1:n,1:nx)::f1,f2,f3,f4,f5,f6,f7

  real(kind=8),dimension(:,:,:),allocatable::u,dudt,w1,w2,w3,w4,w5,w6, ureal
  real(kind=8),dimension(:,:,:),allocatable::f1,f2,f3,f4,f5,f6,f7

  integer::iter=0,icell,i,j,ivar
  ...
  !=================================
  !write(filename,"(A5,A10,I5.5)")"hydro_",title,iter/10
  ...
  do while(t < tend)
    ...
    if(integrator=='RKF')then
     ...
     if (t > accum_time ) then
       ...
       write(filename,"(A,A,A,I5.5)")base_folder,TRIM(title),"_nodes_",iter
       open(10,file=TRIM(filename)//".dat",form='formatted')
       do icell=1,nx
         do i = 1,n
       ...
       write(filename,"(A,A,A,I5.5)")base_folder,TRIM(title),"_modes_",iter
  ...
  !==========================
  !write(filename,"(A5,A10,I5.5)")"recon",title,99999
  !open(10,file=TRIM(filename)//".dat",form='formatted')
  !do icell=1,nx
  !  do i = 1,n
  !   xcell = (dble(icell)-0.5)*dx + dx/2*chsi_quad(i)
  !   write(10,'(7(1PE12.5,1X))')xcell,(ureal(ivar,icell,i),ivar=1,nvar)
  ! end do
  !end do
  !close(10)

  !write(filename,"(A5,A10,I5.5)")"hydro",title,99999
  !open(10,file=TRIM(filename)//".dat",form='formatted')
  !do icell=1,nx
  !   xcell=(dble(icell)-0.5)*dx
  !   call compute_primitive(u(1:nvar,1,icell),ww,gamma,nvar)
...
!subroutine limiter_in_c(u_lim)
!  use iso_c_binding
!  use dg_commons
!  implicit none
!  real(kind=8),dimension(1:nvar,1:n,1:nx)::u_lim
  !real(kind=8), allocatable::u_temp(:,:,:)
  !type (c_ptr) :: u_d
  !integer::a,b,c,d
  !print*,"Fortran function sending something to C."

  !allocate(u_temp(1:nvar,1:n,1:nx))
  !u_temp = u_lim
  !call get_allocation(nvar,n,nx)
  !call set_pointers(u_d)
!  call array(u_lim,nvar,nx,n)
  ! do the limiting outside
!  write(*,*)"Fortran function received something from C."
  !u_lim = u_temp
  !deallocate(u_temp)

!end subroutine limiter_in_c

subroutine limiter_ml(u)
  use dg_commons
  implicit none
  real(kind=8),dimension(1:nvar,1:n,1:nx)::u
  !================================================================
